...
export const searchSongs: Function = (
  query: string | null,
  paginageStr?: string
) => {
  return (dispatch: Function) => {
    query = query && query.replace(" ", "%20");
    const searchStr = paginageStr
      ? paginageStr
      : `${baseString}/search?type=track&q=${query}`;
    ...
    fetch(searchStr, {
      ...
      .then((resp) => {
        if (resp.status === 401) delete localStorage.access_token;
        return resp.json();
      })
      .then((data) => {
...
export const searchArtists: Function = (
  ...
  return (dispatch: Function) => {
    query = query && query.replace(" ", "%20");
    const searchStr = paginateStr
      ? paginateStr
      : `${baseString}/search?type=artist&q=${query}`;
