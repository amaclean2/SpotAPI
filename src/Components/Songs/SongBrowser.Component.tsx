    ...
    if (!!songs.songs) {
      return (
        <div>
          ...
          {songs.songs &&
            songs.songs.map((song: any, key: number) => (
              <SongThumb key={`song_${key}`} song={song} />
            ))}
          {showPag()}
        </div>
      );
  ...
  const showPag = () => {
    return (
      <div className="pagination flex-container">
        {songs.previous ? (
          <button onClick={() => handlePag(songs.previous, "songs")}>
            Previous
          </button>
        ) : (
          ""
        ...
        {songs.next ? (
          <button onClick={() => handlePag(songs.next, "songs")}>Next</button>
        ) : (
          ""
        )}
      </div>
