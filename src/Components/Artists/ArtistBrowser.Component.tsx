    ...
    if (!!artists.artists) {
      return (
        <div>
          ...
          {artists.artists &&
            artists.artists.map((artist: any, key: number) => (
              <ArtistThumb key={`artist_${key}`} artist={artist} />
            ))}
          {showPag()}
        </div>
      );
  ...
  const showPag = () => {
    return (
      <div className="pagination flex-container">
        {artists.previous ? (
          <button onClick={() => handlePag(artists.previous, "artists")}>
            Previous
          </button>
        ) : (
          ""
        ...
        {artists.next ? (
          <button onClick={() => handlePag(artists.next, "artists")}>
            Next
          </button>
        ) : (
          ""
